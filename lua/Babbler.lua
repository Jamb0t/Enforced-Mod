...
Babbler.kMass = 15
Babbler.kRadius = .25
Babbler.kLinearDamping = 0.9 //0
Babbler.kRestitution = 0.65
Babbler.kAttackRange = 1
Babbler.kFov = 160

local kTargetSearchRange = 16
local kAttackRate = 0.5 //0.37
local kLifeTime = 90

local kUpdateMoveInterval = 0.5
local kUpdateAttackInterval = 0.66
local kMinJumpDistance = 1
local kMaxJumpDistance = 8
local kBabblerRunSpeed = 9
local kVerticalJumpForce = 4
local kMaxJumpForce = 13
local kMinJumpForce = 3
local kTurnSpeed = math.pi * 2
...
if Server then
    ...
    function Babbler:MoveRandom()
        ...
        if self.moveType == kBabblerMoveType.None and self:GetIsOnGround() then
            ...
            elseif owner and activeWeapon and activeWeapon:isa("BabblerAbility") and not activeWeapon:GetRecentlyThrown() and (owner:GetOrigin() - self:GetOrigin()):GetLength() <= 4 then    
                self:SetMoveType(kBabblerMoveType.Wag, owner, owner:GetOrigin())
                
            elseif owner and not owner:isa("Commander") and (self:GetOrigin() - owner:GetOrigin()):GetLength() > 16 then                
    ...
    function Babbler:UpdateMove(deltaTime)
        ...
        elseif self.moveType == kBabblerMoveType.Move or self.moveType == kBabblerMoveType.Cling then
            ...
            elseif self:GetIsOnGround() then
                ...
                if self.timeLastJump + kUpdateMoveInterval < Shared.GetTime() then

                    local targetPosition = self.targetPosition or (self:GetTarget() and self:GetTarget():GetOrigin())                    
                    if targetPosition then
                    
                        local distance = math.max(0, ((self:GetOrigin() - targetPosition):GetLength() - Babbler.kAttackRange))
        ...
        if self.physicsBody and not self.doesGroundMove then
            ...
            if self.lastVelocity ~= nil then
                ...
                if delta:GetLengthSquaredXZ() > 0.0001 then
					...
					local trace = Shared.TraceCapsule(self.lastOrigin, endPoint, Babbler.kAttackRange * 0.5, 0, CollisionRep.Damage, PhysicsMask.Bullets, EntityFilterOneAndIsa(self, "Babbler")) 
